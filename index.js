...
if (!fs.existsSync(`./${file.pathFile + '/creds.json'}`)) {
    ...
    if (opcion == '1') {
        objeto = { connectType: '1', phoneNumber: '' };
    } else if (opcion == '2') {
        objeto = { connectType: '2', phoneNumber: (await readline.question('\n\x1b[1;31m~\x1b[1;37m> ¿Cual es el numero que desea asignar como Bot?\n: ')).trim() };
        readline.close();
    }
}
...
app.listen(port, () => { console.log(`Servidor HTML activo en http://localhost:${port}`) })
